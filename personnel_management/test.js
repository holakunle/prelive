...
function isNewStudy(study) {
    const studyDate = new Date(study.MainDicomTags.StudyDate); // Format: YYYYMMDD
    const now = new Date();
    const diff = (now - studyDate) / (1000 * 60 * 60); // Hours difference
    return diff <= 24; // True if within 24 hours
}
...
const getters = {
    ...
    studiesWithNewFlag: state => {
        return state.studies.map(study => ({
          ...study,
          isNew: isNewStudy(study)
        }));
    }
}
